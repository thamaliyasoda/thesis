...
$dd2  = "";
	...
	{
		$dd1 = date( 'Y-m-d', strtotime($_POST['first_date']) );
		$dd2 = date( 'Y-m-d', strtotime($_POST['second_date']) );
		$d1 = date( 'Y-m-d', strtotime($_POST['first_date']) );
		$d2 = date( 'Y-m-d', strtotime($_POST['second_date']));
		echo '<script> document.getElementById("first_date").value = "'.$dd1.'"; </script>' ;
		if($d2 >= $d1)
		{
			$all_array = [];
			...
			WHERE
			ev.event_status = 1  AND (ev.event_end_datetime <= '$d2' AND ev.event_start_datetime >= '$d1')
			...
			WHERE
			practice_status = 1 AND (practice_date BETWEEN '$d1' AND '$d2')
			ORDER BY
			...
			if(mysqli_num_rows($practiceResult ) > 0)
			{
				while($sessions = mysqli_fetch_array($practiceResult ))
				{
					...
					$tablebody .=' <tr> 
									...
									<td>'.$sessions['practice_date'].' '.date( 'g:i A', strtotime($sessions['practice_start_time']) ).' - '.date( 'g:i A', strtotime($sessions['practice_end_time']) ).'</td>
									 <td>'.$sessions['practice_location'].'</td>
									<td>
										<a href="view_practice_session.php?session='.$sessions['practice_id'].'" class="view"  ><i class="fas fa-eye" data-toggle="tooltip" title="View"></i></a>
									</td>
								</tr>';
				}
			}
				...
				$success_msg ="<label style='color:red; font-size:15px' id='success_msg' >End date cannot be larger than start date ! </label>";
		}	
	}
                 ...
                 <ul class="list-unstyled components">
                    ...
                    <li class="active">
                        ...
                        <ul class="collapse list-unstyled" id="reportSubmenu">
							...
							<li class="active"><a href="report_schedule_for_time_period.php"><i class="fas fa-calendar-alt"></i>Schedule For a Duration</a></li>
...
<div class="d-flex justify-content-right"><a class="nav-link" href="logout.php"><i class="fas fa-sign-out-alt"></i> Log Out <span class="sr-only">(current)</span></a><a href="#" class="btn btn-sm btn-dark navbar-btn" onclick="history.back()" ><i class="fas fa-arrow-left"></i> <span> Back</span></a></div>
		...
		<div class="table-wrapper">
			<div class="table-title">
						...
						<h2><b>Schedule</b> for the time period</h2>
		...
		<div class="row">
						...
						<form method="post">
							<div class="row">
								<div class="col-sm-4">
									...
									<input type="date" class="form-control" name="first_date" id="first_date"  required >
									</div>
								</div>
								<div class="col-sm-4">
									<div class="form-group">
									<label>End Date</label>
									<input type="date" class="form-control" name="second_date" id="second_date"   required >
									</div>
								</div>
								<div class="col-sm-4">
									<div class="form-group">
										<input type="submit" name="get_student" class="btn btn-primary" value="Get schedule for the period">
									</div>
								</div>									
							</div>
						</div>
		 ...
		 <script> document.getElementById("first_date").value = "<?=$dd1; ?>"; </script>		
		 <script> document.getElementById("second_date").value = "<?=$dd2; ?>"; </script>		
</body>
</html>
<?php 
}else
